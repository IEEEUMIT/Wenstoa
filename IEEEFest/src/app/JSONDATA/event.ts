export var event = 
[{   "id": 7, 
    "name": "Ravzion",
    "type": "Life is full of riddles, So while exploring and solving one another,",
    "type2":"don't forget to enjoy your journey too",
    "tagline": "Wander wisely",
    "member":"free",
    "nonmember":"free",
    "date" : "2",
    "time":[
        "round 1: 11 to 12",
        "round 2: 12 to 1",
    ],
  
    "eventincharge":
    [
        {
            "name":"Tanvee Joshi",
            "emailid":"tanvee03joshi@gmail.com",
            "number":"9028249474",
            
        },

        {
            "name":"Kshitija Karande",
            "emailid":"kshitijaak2502@gmail.com",
            "number":"8788354162",
        },

        {
            "name":"Pratiksha Barman",
            "emailid":" pratikshabarman208@gmail.com",
            "number":"7977605183",
        },
    ],
   "prizes": ["will reveal soon!"],
   "rules" : ["In case of any discrepancy, the organizers will have a final say."],
},
{    "id":6 ,
    "name":"Mahfil-E-Taxi",
    "type": "*Happiness is*....a taxi appearing just when you need it...So board the taxi",
    "type2":"Dil ki dastaan sunnao tumhari Jarur aao mehfil me humari",
    "tagline": "Raah me milte lafz",
    "member":"20",
    "nonmember":"25",
    "date" : "56 65",
    "time":[
        "round 1: 12 to 13",
        "round 2: 11 to 16",
    ],
    "eventincharge":
    [
        {
            "name":"Tanvee Joshi",
            "emailid":"tanvee03joshi@gmail.com",
            "number":"9028249474",
        },

        { "name":"Kshitija Karande",
        "emailid":"kshitijaak2502@gmail.com",
        "number":"8788354162",
            
        },

        {
            "name":"Pratiksha Barman",
            "emailid":" pratikshabarman208@gmail.com",
            "number":"7977605183",
            
       },
    ],
  "prizes": ["Will reveal soon!!"],
  "rules" : ["hgfcbnhd hgsxfggrwwx jhgfghhy jhyreecbhj jjytvvbnjjy "],
}, 


{
    "id": 4,   
    "name":"Web Srujana",
    "type": "Give your  touch to design and make websites come alive beacause Great",
    "type2":"web design is more than pretty pictures",
    "tagline": "Awaken Your Inner Srujana ",
    "member":"Free",
    "nonmember":"Free",
    "date" : "56 65",
    "time":[
        "round 1: 12 to 13",
        "round 2: 11 to 16",
    ],
    "eventincharge":
    [
        {
            "name":"Tanvee Joshi",
            "emailid":"tanvee03joshi@gmail.com",
            "number":"9028249474",
        },

        { "name":"Kshitija Karande",
        "emailid":"kshitijaak2502@gmail.com",
        "number":"8788354162",
            
        },

        {
            "name":"Pratiksha Barman",
            "emailid":" pratikshabarman208@gmail.com",
            "number":"7977605183",
            
       },
    ],
  "prizes": ["Will reveal soon!!"],
  "rules" : ["hgfcbnhd hgsxfggrwwx jhgfghhy jhyreecbhj jjytvvbnjjy "]
}, 
{    "id" : 5,
    "name":"MatarGhashti",
    "type": "Life toh hai out of control, but we are here to take full control of your entertainment.",
    "type2":"A fun event, with games of a kind that will help you create memories of a lifetime!",
    "tagline": "Bazm -e-yaaraan",
    "member":"Free",
    "nonmember":"Free",
    "date" : "56 65",
    "time":[
        "round 1: 12 to 13",
        "round 2: 11 to 16",
    ],
    "eventincharge":
    [
        {
            "name":"Tanvee Joshi",
            "emailid":"tanvee03joshi@gmail.com",
            "number":"9028249474",
        },

        { "name":"Kshitija Karande",
        "emailid":"kshitijaak2502@gmail.com",
        "number":"8788354162",
            
        },

        {
            "name":"Pratiksha Barman",
            "emailid":"pratikshabarman208@gmail.com",
            "number":"7977605183",
            
       },
    ],
  "prizes": ["Will reveal soon!!"],
  "rules" : ["hgfcbnhd hgsxfggrwwx jhgfghhy jhyreecbhj jjytvvbnjjy "]
}, 
{   "id" : 1,
    "name":"Bombay Circuits",
    "type": "The best error message is the one that never shows up.Experience name everyone  ",
    "type2":"gives to their mistakes So come and build the experience you want.",
    "tagline": "Think.Code.Execute",
    "member":"500",
    "nonmember":"Free",
    "date" : "Free",
    "time":[
        "round 1: 12 to 13",
        "round 2: 11 to 16",
    ],
    "eventincharge":
    [
        {
            "name":"Tanvee Joshi",
            "emailid":"tanvee03joshi@gmail.com",
            "number":"9028249474",
        },

        { "name":"Kshitija Karande",
        "emailid":"kshitijaak2502@gmail.com",
        "number":"8788354162",
            
        },

        {
            "name":"Pratiksha Barman",
            "emailid":"pratikshabarman208@gmail.com",
            "number":"7977605183",
            
       },
    ],
  "prizes": ["Will reveal soon!"],
  "rules" : ["hgfcbnhd hgsxfggrwwx jhgfghhy jhyreecbhj jjytvvbnjjy "]
}, 
{   "id" : 2,
    "name":"NaukariCentral",
    "type": "Best practices are those practices that generally produce the ",
     "type2": "best results or minimize risk.So come and give a try!!",
    "tagline": "On The Hunt For The One",
    "member":"40",
    "nonmember":"45",
    "date" : "56 65",
    "time":[
        "round 1: 12 to 13",
        "round 2: 11 to 16",
    ],
    "eventincharge":
    [
        {
            "name":"Tanvee Joshi",
            "emailid":"tanvee03joshi@gmail.com",
            "number":"9028249474",
        },

        { "name":"Kshitija Karande",
        "emailid":"kshitijaak2502@gmail.com",
        "number":"8788354162",
            
        },

        {
            "name":"Pratiksha Barman",
            "emailid":" pratikshabarman208@gmail.com",
            "number":"7977605183",
            
       },
    ],
  "prizes": [" ₹500"],
  "rules" : ["hgfcbnhd hgsxfggrwwx jhgfghhy jhyreecbhj jjytvvbnjjy "]
}, 

{    "id" : 3,
    "name":"TechTalkies",
    "type": "An opportunity to showcase your knowledge should never be wasted.So what",
    "type2":"are you waiting for? Tech talkies gives you the platform to show off your skills",
    "tagline": "Unleashing Ideas Speak your mind",
    "member":"40",
    "nonmember":"45",
    "date" : "56 65",
    "time":[
        "round 1: 12 to 13",
        "round 2: 11 to 16",
    ],
    "eventincharge":
    [
        {
            "name":"Tanvee Joshi",
            "emailid":"tanvee03joshi@gmail.com",
            "number":"9028249474",
        },

        { "name":"Kshitija Karande",
        "emailid":"kshitijaak2502@gmail.com",
        "number":"8788354162",
            
        },

        {
            "name":"Pratiksha Barman",
            "emailid":" pratikshabarman208@gmail.com",
            "number":"7977605183",
            
       },
    ],
  "prizes": [" ₹500"],
  "rules" : ["hgfcbnhd hgsxfggrwwx jhgfghhy jhyreecbhj jjytvvbnjjy "]
},
{    "id" : 8,
    "name":"One of them is Lying ",
    "type": "Kuch toh gadbad hai DAYA.... Everything is a mystery until you unlock it.",
    "type2":"Be a detective for a day. Unleash the power of your mind",
    "tagline": "Ghum Naam Hai Koi,Badnaam Hai Koi",
    "member":"20 per person",
    "nonmember":"20 person",
    "date" : "56 65",
    "time":[
        "round 1: 12 to 13",
        "round 2: 11 to 16",
    ],
    "eventincharge":
    [
        {
            "name":"Tanvee Joshi",
            "emailid":"tanvee03joshi@gmail.com",
            "number":"9028249474",
        },

        { "name":"Kshitija Karande",
        "emailid":"kshitijaak2502@gmail.com",
        "number":"8788354162",
            
        },

        {
            "name":"Pratiksha Barman",
            "emailid":" pratikshabarman208@gmail.com",
            "number":"7977605183",
            
       },
    ],
  "prizes": ["Will reveal soon!"],
  "rules" : ["hgfcbnhd hgsxfggrwwx jhgfghhy jhyreecbhj jjytvvbnjjy "]
}, 
{
    "id" : 9,
"name":"Phir Hera Pheri ",
"type": "Treasure Hunt",
"tagline": "Paisa Hi Paisa Hoga",
"member":"25 per person",
"nonmember":"25 per person",
"date" : "56 65",
"time":[
    "round 1: 12 to 13",
    "round 2: 11 to 16",
],
"eventincharge":
[
    {
        "name":"Tanvee Joshi",
        "emailid":"tanvee03joshi@gmail.com",
        "number":"9028249474",
    },

    { "name":"Kshitija Karande",
    "emailid":"kshitijaak2502@gmail.com",
    "number":"8788354162",
        
    },

    {
        "name":"Pratiksha Barman",
        "emailid":" pratikshabarman208@gmail.com",
        "number":"7977605183",
        
   },
],
"prizes": [" ₹400"],
"rules" : ["hgfcbnhd hgsxfggrwwx jhgfghhy jhyreecbhj jjytvvbnjjy "]
}, 

]